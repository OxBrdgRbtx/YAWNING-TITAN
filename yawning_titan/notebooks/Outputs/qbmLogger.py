...
class qbmLogger:
    ...
    def weightsToTxt(self,agent):
        ...
        try:
            outPath = os.path.join(self.resultsDir,f'Step_{agent.step}')
            os.mkdir(outPath)
            agent.saveWeights(outPath)
            agent.log.resultsDir = outPath
            agent.exportResults(showFigs=False,saveFigs=False,storeMeta=False)
        except:
