...
from yawning_titan.agents.QBMagent import QBMAgent
...
import time
count = 256
print("count ",count, " time ", time.asctime(), " starting")
while True:
    start_time = time.time()
    log = agent.learn(nSteps=count)
    print("count ", count, " time ", (time.time() - start_time))
    results = QBMResults(agent,'DBMtest_Simulate'+str(count))
    results.toExcel()
    results.plot(showFigs=True,saveFigs=True)
    count = count * 2