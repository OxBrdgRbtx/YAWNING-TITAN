...
#   updateLog           - Log the current training step
    ...
    def __init__(self,env: GenericNetworkEnv):
        ...
        self.SimulateAnneal = False # Simulate or use D-Wave
    ...
    def evaluateQBM(self,state,action,SimulateAnneal=True):
        ...
        if SimulateAnneal:
            ...
            results = self.sampler.sample(Hamiltonian,num_reads=10,beta_range=[beta0, self.beta])
        else:
            beta0 = min(0.1,self.beta/5)
            #self.sampler=EmbeddingComposite(DWaveSampler())
            results = self.sampler.sample(Hamiltonian,num_reads=10)
